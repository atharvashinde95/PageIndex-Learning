...
---------------------
Replicates the full PageIndex index-generation pipeline:

  Phase 1  – TOC detection (scan first N pages)
  Phase 2a – TOC with page numbers   → toc_transformer + toc_index_extractor
  Phase 2b – TOC without page numbers → toc_transformer + add_page_number_to_toc
  Phase 2c – No TOC                  → generate_toc_init + generate_toc_continue
  Phase 3  – Verify title appearances; fix or fall back
  Phase 4  – Recursive split for oversized nodes
  Phase 5  – post_processing: flat list → nested tree
  Phase 6  – Enrich: node_id, summary, node text, doc description

All LLM calls use core.llm_client.call_llm().
No external API. No vector DB.
"""
...
TOC_CHECK_PAGES   = 20      # How many pages to scan for a TOC
MAX_PAGES_PER_NODE  = 10    # Split node if it spans more pages than this
MAX_TOKENS_PER_NODE = 20_000  # Split node if it contains more tokens than this
CHUNK_SIZE_PAGES  = 15      # Pages fed to the LLM in one generate_toc call
ENCODING_NAME     = "cl100k_base"   # tiktoken encoding (model-agnostic safe default)
...
def add_page_number_to_toc(
    ...
    """
    total = len(pages)
    # Build a flat title list to resolve
    unresolved = {e["structure"]: e["title"] for e in toc_entries}
    resolved: Dict[str, int] = {}   # structure → start_index
...
def generate_toc_from_content(
    ...
    """
    When no TOC exists, build the hierarchy from scratch.
    Mirrors generate_toc_init() + generate_toc_continue() in the original code.
    """
    total = len(pages)
...
Rules:
- Continue the same numbering scheme (e.g. if last was "3.2", continue from there).
...
# ═════════════════════════════════════════════════════════════════════════════
#  PHASE 3 – Verification: check title appears on assigned start page
# ═════════════════════════════════════════════════════════════════════════════
...
def fix_incorrect_sections(
    ...
    Mirrors fix_incorrect_toc_with_retries() from the original code.
    """
    for attempt in range(1, max_retries + 1):
        ...
        tagged = build_tagged_text(pages)   # full doc for correction
...
Document (all pages):
{tagged[:12000]}"""   # Truncate to avoid token overflow
        ...
        try:
            ...
            incorrect = []   # Optimistic — accept LLM fix
            return fixed
        except json.JSONDecodeError:
            logger.warning("fix_incorrect_sections attempt %d: JSON parse failed", attempt)

    # Fallback: return incorrect sections as-is
    return incorrect
...
def split_large_nodes(
    ...
    for sec in sections:
        ...
        if page_span > max_pages and token_count > max_tokens:
            ...
            # Re-number sub-sections under the parent's structure
...
def list_to_tree(sections: List[Dict]) -> List[Dict]:
    ...
    Mirrors list_to_tree() + build_tree_from_nodes() from the original codebase.
    """
    root_nodes: List[Dict] = []
    # stack entries: (node_dict, depth)
    ...
    for sec in sections:
        ...
        # Pop all stack entries at same or deeper level
...
def assign_node_ids(tree: List[Dict], counter: List[int] = None) -> None:
    ...
    Mirrors write_node_id() from the original code.
...
def build_index(
    ...
    Steps:
      1  Detect TOC
      2  Extract sections (3 strategies with fallback)
    ...
    if incorrect:
        ...
        # Re-sort by start_index
        sections.sort(key=lambda s: s.get("start_index", 0))
    ...
    logger.info("build_index: complete — %d top-level nodes", len(tree))
    return result
...
{bad_output}"""
    ...
    raw = _strip_json_fences(raw)
    return json.loads(raw)   # If this fails too, let it propagate
...
def _assign_end_indices(sections: List[Dict], total_pages: int) -> None:
    ...
    for i, sec in enumerate(sections):
        if i + 1 < len(sections):
            next_start = sections[i + 1].get("start_index", total_pages)
            sec["end_index"] = max(sec.get("start_index", 1), next_start - 1)
        else:
